...
import {
	...
	guardSettingWrite,
	resolveSetting,
	settingWriteFailureNotice,
} from '../frontend/services/setting-value';
...
describe('settingWriteFailureNotice', () => {
	/**
	 * The toast is the only thing that tells a user why a toggle they just clicked snapped back: the
	 * displayed value comes straight from the config store with no optimistic local state, so a failed
	 * write is otherwise indistinguishable from not having clicked at all.
	 */
